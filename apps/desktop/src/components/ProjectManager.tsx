import { Archive, Check, Pencil, Plus, Trash2, Users, X } from "lucide-react";
import { useState } from "react";
import { Button, IconButton, Input, Select, useToast } from "@task-manager/ui";
import type { ProjectMember } from "@task-manager/domain";
    ...
    <div className="entity-manager">
      ...
      <ul className="entity-list">
        {projects.map((project) => (
          <li
            key={project.id}
            className={membersProjectId === project.id ? "entity-row entity-row--members" : "entity-row"}
          >
            <span
            ...
            <div className="entity-row__actions">
              <IconButton label={`成员 ${project.name}`} onClick={() => void toggleMembers(project.id)}>
                <Users size={15} />
              </IconButton>
              <IconButton label={`编辑 ${project.name}`} onClick={() => startEdit(project.id)}>
                <Pencil size={15} />
              </IconButton>
              <IconButton label={`归档 ${project.name}`} onClick={() => void archiveProject(project.id)}>
                <Archive size={15} />
              </IconButton>
              <IconButton
              ...
              <div className="project-member-editor">
                  ...
                  {members.length === 0 ? (
                    ...
                    members.map((member) => (
                      <div key={member.id} className="project-member-item">
                        ...
                        <span>{member.role === "admin" ? "管理员" : member.role === "editor" ? "编辑" : "查看"}</span>
                        <IconButton
                          size="sm"
                ...
                <div className="project-member-add">
                  ...
                  <Select
                    label="角色"
                    value={memberRole}
                    onChange={(event) => setMemberRole(event.target.value as "viewer" | "editor" | "admin")}
                    options={[
